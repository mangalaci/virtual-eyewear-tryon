...
def save(img, path):
    ...
    # Crop to the bounding box of substantially opaque pixels (alpha > 10)
    # so the image width equals the frame front width (no blank padding).
    alpha = final.getchannel('A').point(lambda p: 255 if p > 10 else 0)
    bbox = alpha.getbbox()
    if bbox:
